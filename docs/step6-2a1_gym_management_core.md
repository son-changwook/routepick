# Step 6-2a1: GymService 구현 - 핵심 관리 기능

> 체육관 핵심 관리 서비스 - CRUD, 검증, 상태 관리  
> 생성일: 2025-08-21  
> 단계: 6-2a1 (Service 레이어 - 체육관 도메인 핵심)  
...
## 🏢 GymService - 체육관 핵심 관리

### GymService.java (Part 1 - 핵심 관리)
```java
...
- **지점 생성**: 지점 정보 및 좌표 관리
- **지점 수정**: 좌표, 운영시간, 주소 수정
- **한국 좌표 검증**: 위도 33.0~38.6, 경도 124.0~132.0
- **중복 방지**: 동일 체육관 내 지점명 중복 방지
...
- **벽면 정보**: `@Cacheable("branch-walls")` - 지점별 벽면 캐싱
- **캐시 무효화**: `@CacheEvict` 적절한 시점에 캐시 갱신
...
### 입력 검증
- **필수값 검증**: null 체크 및 빈 문자열 검증
- **좌표 범위**: 한국 영토 내 좌표만 허용
- **비즈니스 규칙**: 체육관명, 지점명, 벽면명 중복 방지
...
*step6-2a1 완성: 체육관 핵심 관리 기능 설계 완료*